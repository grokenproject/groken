...
Placeholders: token name `Groken`, symbol `GRKN`. One chain family: Base (8453) and Base Sepolia (84532). This repository refuses chain id 8453. No mainnet addresses are published here because none have been created by this repository. Base Sepolia (84532) TESTNET addresses and tx hashes are in `WALLETS.md`. A TESTNET Uniswap V2 GRKN/WETH pair exists; seed ETH is not a valuation. The original mock locker is unchanged. No mainnet AMM.
...
Immutable allowlist at deploy (empty allowlist is valid). Never an AMM. Hard-reverts transfers to constructor-frozen project addresses (including `ImmutableLPLock` when wired in the launch batch). `proposeTransfer(ListingFee | MMInventory)` with a 7-day delay and one pending proposal. `executeTransfer` reverts at or after start+90d (no post-kill execute; pending-before-90 die). `sendToDead` is proposer-only until start+90d, then permissionless remainder-to-dead. After start+90d, new proposes revert and `sendRemainderToDead` is permissionless. Unused GRKN goes to dead. No leftover-deal exception.
